...
def save_pass():
    ...
    val_website = entry_website.get().ljust(20, " ")
    val_username = entry_username.get().ljust(20, " ")
    val_password = entry_password.get().ljust(20, " ")
    if len(entry_password.get()) > 0 and len(entry_username.get()) > 0 and len(entry_website.get()) > 0:
        ...
        if is_ok:
            with open(file="password.txt", mode="a+") as file:
                file.write(f"{val_website}|{val_username}|{val_password}\n")
    else:
...
def generate_pass():
    ...
    pyperclip.copy(f"Doudles{nums}{sp_chars}")
    print("updated")
